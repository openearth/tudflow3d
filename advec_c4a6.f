...
!    along with this program.  If not, see <http://www.gnu.org/licenses/>.
      ...
      subroutine advecu_C4A6(putout,Uvel,Vvel,Wvel,RHO,Ru,Rp,dr,phiv,dz,
...
c********************************************************************
	...
	real*8 Vvel2(-2:i1+2,-2:j1+2,-2:k1+2),Wvel2(-2:i1+2,-2:j1+2,-2:k1+2)
...
c get stuff from other CPU's
	...
	Uvel2(0:i1,0:j1,0)=Uvel(0:i1,0:j1,0)
...
c get stuff from other CPU's
	...
	Vvel2(0:i1,0:j1,0)=Vvel(0:i1,0:j1,0)
...
!      wwL=(Wvel(i,j,km)+Wvel(ip,j,km))*rhokm
		   if (.false.) then !(k.le.2.or.k.ge.ke-1) then !switch to cds2
			  Ax=1.
			  Bx=0.
			  Cx=0.
			  DDx=1./(Ax+Bx+Cx)		  
...
!		  if (kmm.eq.0.or.km.eq.0) then
!			fac1=0.    ! correction to get cds2
!			fac2=4./5. ! correction to get cds2
!		  else
			fac1=1.
			fac2=1.
!		  endif		  
		  
      uuR=DDx*(Ax*(Uvel2(i,j,k)+Uvel2(ip,j,k))+Bx*(Uvel2(im,j,k)+Uvel2(ipp,j,k))+Cx*(Uvel2(imm,j,k)+Uvel2(ippp,j,k)))
     & *rho(ip,j,k)*Rp(ip)
      uuL=DDx*(Ax*(Uvel2(im,j,k)+Uvel2(i,j,k))+Bx*(Uvel2(imm,j,k)+Uvel2(ip,j,k))+Cx*(Uvel2(immm,j,k)+Uvel2(ipp,j,k)))
     & *rho(i,j,k)*Rp(i)
	  vvR=DDx*(Ax*(Vvel2(i,j,k)+Vvel2(ip,j,k))+Bx*(Vvel2(im,j,k)+Vvel2(ipp,j,k))+Cx*(Vvel2(imm,j,k)+Vvel2(ippp,j,k)))*rhojp
      vvL=DDx*(Ax*(Vvel2(i,jm,k)+Vvel2(ip,jm,k))+Bx*(Vvel2(im,jm,k)+Vvel2(ipp,jm,k))+Cx*(Vvel2(imm,jm,k)+Vvel2(ippp,jm,k)))*rhojm
      wwR=DDx*(Ax*(Wvel2(i,j,k)+Wvel2(ip,j,k))+Bx*(Wvel2(im,j,k)+Wvel2(ipp,j,k))+Cx*(Wvel2(imm,j,k)+Wvel2(ippp,j,k)))*rhokp
      wwL=DDx*(Ax*(Wvel2(i,j,km)+Wvel2(ip,j,km))+Bx*(Wvel2(im,j,km)+Wvel2(ipp,j,km))+Cx*(Wvel2(imm,j,km)+Wvel2(ippp,j,km)))*rhokm	  
     ...
     +                         +
     3 (wwR*fac2* DDz*(Az*(Uvel2(i,j,k)+Uvel2(i,j,kp))+fac1*Bz*(Uvel2(i,j,km)+Uvel2(i,j,kpp))+Cz*(Uvel2(i,j,kmm)+Uvel2(i,j,kppp)))-
     3 numdif*
     3 ABS(wwR)* (10.*(-Uvel2(i,j,k)+Uvel2(i,j,kp))-5.*(-Uvel2(i,j,km)+Uvel2(i,j,kpp))+(-Uvel2(i,j,kmm)+Uvel2(i,j,kppp))) -
     3 (wwL*fac2* DDz*(Az*(Uvel2(i,j,km)+Uvel2(i,j,k))+fac1*Bz*(Uvel2(i,j,kmm)+Uvel2(i,j,kp))+Cz*(Uvel2(i,j,kmmm)+Uvel2(i,j,kpp)))-
     3 numdif*
     ...
     +                         )
      ...
      subroutine advecv_C4A6(putout,Uvel,Vvel,Wvel,RHO,Ru,Rp,dr,phiv,dz,
     +                  i1,j1,k1,ib,ie,jb,je,kb,ke,rank,px,numdif,periodicx,periodicy)
      implicit none
c
c********************************************************************
...
c********************************************************************
	...
	real*8 Uvel2(-2:i1+2,-2:j1+2,-2:k1+2),Wvel2(-2:i1+2,-2:j1+2,-2:k1+2)
...
!		  endif
     ...
     +                         +
     3 (wwR*fac2* DDz*(Az*(Vvel2(i,j,k)+Vvel2(i,j,kp))+fac1*Bz*(Vvel2(i,j,km)+Vvel2(i,j,kpp))+Cz*(Vvel2(i,j,kmm)+Vvel2(i,j,kppp)))-
     3 numdif*
     3 ABS(wwR)* (10.*(-Vvel2(i,j,k)+Vvel2(i,j,kp))-5.*(-Vvel2(i,j,km)+Vvel2(i,j,kpp))+(-Vvel2(i,j,kmm)+Vvel2(i,j,kppp))) -
     3 (wwL*fac2* DDz*(Az*(Vvel2(i,j,km)+Vvel2(i,j,k))+fac1*Bz*(Vvel2(i,j,kmm)+Vvel2(i,j,kp))+Cz*(Vvel2(i,j,kmmm)+Vvel2(i,j,kpp)))-
     3 numdif*
...
!     4   / ( 2.0*Rp(i) )
     ...
     +                         )
      ...
      subroutine advecw_C4A6(putout,Uvel,Vvel,Wvel,RHO,Ru,Rp,dr,phiv,dz,
     +                  i1,j1,k1,ib,ie,jb,je,kb,ke,rank,px,numdif,periodicx,periodicy)
      implicit none
c
c********************************************************************
...
c********************************************************************
	...
	real*8 Uvel2(-2:i1+2,-2:j1+2,-2:k1+2),Vvel2(-2:i1+2,-2:j1+2,-2:k1+2)
