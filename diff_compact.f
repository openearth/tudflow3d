...
c*****************************************************************
     ...
     +         eppo,epmo,epop,epom,drp,dzi,divergentie
      ...
      integer n,t
	  !real fcl2(0:i1,0:px*jmax+1,0:k1)
	...
	ENDIF	
     ...
     + * MIN(fcl2(i,j,k),fcl2(ip,j,k),fcl2(ip,jp,k),fcl2(i,jp,k))	 
      epmo = 0.25 * (
     +   ekm(i,j,k) + ekm(ip,j,k) + ekm(ip,jm,k) + ekm(i,jm,k)  )
     + * MIN(fcl2(i,j,k),fcl2(ip,j,k),fcl2(ip,jm,k),fcl2(i,jm,k))	 
      epop = 0.25 * (
     +   ekm(i,j,k) + ekm(ip,j,k) + ekm(ip,j,kp) + ekm(i,j,kp)  )
     + * MIN(fcl2(i,j,k),fcl2(ip,j,k),fcl2(ip,j,kp),fcl2(i,j,kp))	 
      epom = 0.25 * (
     +   ekm(i,j,k) + ekm(ip,j,k) + ekm(ip,j,km) + ekm(i,j,km)  )
     + * MIN(fcl2(i,j,k),fcl2(ip,j,k),fcl2(ip,j,km),fcl2(i,j,km))	 
     ...
     1 (  Rp(ip) * ekm(ip,j,k) * fcl2(ip,j,k)*CNx *
!     1 (  Rp(ip) * ekm(ip,j,k)*CNx  *	 
     1            ((Uvel(ip,j,k) - Uvel(i,j,k) ) / ( dr(ip) ) - 1./3.*divvR) -
     1    Rp(i ) * ekm(i,j,k) * fcl2(i,j,k)*CNx *
!     1    Rp(i ) * ekm(i,j,k)*CNx * 	 
     ...
     3          ) ) * dzi
     +              -
     4   CNy*(ekm(i,j,k) + ekm(ip,j,k)) * MIN(fcl2(i,j,k),fcl2(ip,j,k)) * ( Uvel(i,j,k) +
...
!     1        ) * Ru(im) ) / ( Rp(i) * dr(i) )
     +              +
     2 ( CNy*ekm(i,jp,k)*fcl2(i,jp,k) * (   (Uvel(i,jp,k) + Uvel(im,jp,k)) / 2.0
!     2 ( CNy*ekm(i,jp,k) * (   (Uvel(i,jp,k) + Uvel(im,jp,k)) / 2.0	 
     2                   + (Vvel(i,jp,k) - Vvel(i,j,k)  ) / (phiv(jp)-phiv(j))
     2                   - 1./3.*divvR*Rp(i)
     2                 )             -
     2   CNy*ekm(i,j,k)*fcl2(i,j,k)  * (   (Uvel(i,j,k)  + Uvel(im,j,k) ) / 2.0
...
c*****************************************************************
	...
	dzi=1./dz
     ...
     + * MIN(fcl2(i,j,k),fcl2(i,j,kp),fcl2(im,j,k),fcl2(im,j,kp))	 
      eopp = 0.25 * (
     +   ekm(i,j,k) + ekm(i,j,kp) + ekm(i,jp,k) + ekm(i,jp,kp) )
     + * MIN(fcl2(i,j,k),fcl2(i,j,kp),fcl2(i,jp,k),fcl2(i,jp,kp))	 
      eomp = 0.25 * (
     +   ekm(i,j,k) + ekm(i,j,kp) + ekm(i,jm,k) + ekm(i,jm,kp) )
     + * MIN(fcl2(i,j,k),fcl2(i,j,kp),fcl2(i,jm,k),fcl2(i,jm,kp))	 
     ...
     +             +
     3 ( CNz*ekm(i,j,kp)*fcl2(i,j,kp) * (CNz*(Wvel(i,j,kp) - Wvel(i,j,k ))*dzi - 1./3.*divvR ) -
     3   CNz*ekm(i,j,k )*fcl2(i,j,k) * (CNz*(Wvel(i,j,k)  - Wvel(i,j,km))*dzi - 1./3.*divvL )	 
...
c
     ...
     1 ( CNx*Ru(i)  * (ekh(i,j,k)+ekh(ip,j,k)) * MIN(fc_local(i,j,k),fc_local(ip,j,k)) *
     1            (putin2(ip,j,k)-putin2(i,j,k) ) / (Rp(ip) - Rp(i))   -
     1   CNx*Ru(im) * (ekh(i,j,k)+ekh(im,j,k)) * MIN(fc_local(i,j,k),fc_local(im,j,k)) *
     1            (putin2(i,j,k) -putin2(im,j,k)) / (Rp(i) - Rp(im))    )
     1 * Rpdr_i !/ ( Rp(i) * dr(i) )
     +              +
     2 (    CNy*(ekh(i,j,k)+ekh(i,jp,k)) * MIN(fc_local(i,j,k),fc_local(i,jp,k)) 
     2 * (putin2(i,jp,k)-putin2(i,j,k) )/(Rp(i)*(phip(jp)-phip(j ))) -
     2      CNy*(ekh(i,j,k)+ekh(i,jm,k)) * MIN(fc_local(i,j,k),fc_local(i,jm,k))
     2 * (putin2(i,j,k) -putin2(i,jm,k))/(Rp(i)*(phip(j )-phip(jm)))  )
     2 / ( Rp(i) *(phiv(j )-phiv(jm)) )
     +              +
     3 (CNz*(ekh(i,j,k)+ekh(i,j,kp)) * MIN(fc_local(i,j,k),fc_local(i,j,kp)) 
     3 * (putin2(i,j,kp)-putin2(i,j,k) ) -
     3  CNz*(ekh(i,j,k)+ekh(i,j,km)) * MIN(fc_local(i,j,k),fc_local(i,j,km)) 
