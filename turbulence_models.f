...
!       include 'common.txt'
      !include 'mpif.h'
	real shear
      real xx,yy,f,dzi,uu,vv,absU,ust,z0,yplus,ww,tauw,tauv,tauu
      integer n,t,tel,kt,n2
      real ebb(0:i1,0:k1)
      real ebf(0:i1,0:k1)
      real Lm2,dRpp_i,dRp_i,divergentie
...
!     3  (       Wvel(i,j,k) -         Wvel(i,j,k-1) ) / ( dz ))   
	...
	enddo
			...
			do j=1,jmax
				...
				ELSEIF (nu_minimum_wall.eq.3) THEN 
					...
					ELSE 
						...
						ekm(i,j,kp) = MAX(ekm(i,j,kp),nu_mol*rr(i,j,kp)*kappa*yplus*(1-exp(-yplus/19.))**2)					
					ENDIF  
				ELSEIF (nu_minimum_wall.eq.4) THEN 
					! ust*ust = nu_t*dUdz --> nu_t = ust*ust/dUdz 
					nu_t = ust*ust/(MAX(absU,1.e-9)/distance_to_bed)
					factor = nu_t/(ekm(i,j,kp)/rr(i,j,kp))
					factor = MAX(factor,1.) 
					ekm(i,j,kp)=factor*ekm(i,j,kp)
				ELSEIF (nu_minimum_wall.eq.5) THEN 
					! OpenFOAM manner: tau = rho*ust*ust and ust=absU/Uplus and tau=nu_t*dUdz combined:
					nu_t = ust*distance_to_bed / (MAX(absU,1.e-9)/MAX(ust,1.e-9)) 
					factor = nu_t/(ekm(i,j,kp)/rr(i,j,kp))
					factor = MAX(factor,1.) 
					ekm(i,j,kp)=factor*ekm(i,j,kp)
				...
				ELSEIF (nu_minimum_wall.eq.13) THEN 
					...
					ELSE 
						...
						ekm(i,j,kp) = nu_mol*rr(i,j,kp)*kappa*yplus*(1-exp(-yplus/19.))**2					
					ENDIF  
				ELSEIF (nu_minimum_wall.eq.14) THEN 
					! ust*ust = nu_t*dUdz --> nu_t = ust*ust/dUdz 
					nu_t = ust*ust/(MAX(absU,1.e-9)/distance_to_bed)
					factor = nu_t/(ekm(i,j,kp)/rr(i,j,kp))
					!factor = MAX(factor,1.) 
					ekm(i,j,kp)=factor*ekm(i,j,kp)
				ELSEIF (nu_minimum_wall.eq.15) THEN 
					! OpenFOAM manner: tau = rho*ust*ust and ust=absU/Uplus and tau=nu_t*dUdz combined:
					nu_t = ust*distance_to_bed / (MAX(absU,1.e-9)/MAX(ust,1.e-9)) 
					factor = nu_t/(ekm(i,j,kp)/rr(i,j,kp))
					!factor = MAX(factor,1.) 
		...
		IF (monopile.eq.1.and.nu_minimum_wall.ge.1) then ! partial slip tau based on log-wall 
			 do k=1,kmax 
			   do j=1,jmax
				distance_to_bed = (Rp(1)-Ru(0))
				ww=0.5*(Wvel(1,j,k)+Wvel(1,j,k-1))
				vv=0.5*(Vvel(1,j,k)+Vvel(1,j-1,k))				
				absU=sqrt(ww*ww+vv*vv)
				ust=0.1*absU
				if (kn_mp>0.) then !walls with rougness (log-law used which can be hydr smooth or hydr rough):
					do tel=1,10 ! 10 iter is more than enough
						z0=kn_mp/30.+0.11*nu_mol/MAX(ust,1.e-9)
						ust=absU/MAX(1./kappa*log(distance_to_bed/z0),2.) !ust maximal 0.5*absU
					enddo
				else
					do tel=1,10 ! 10 iter is more than enough
				...
				ELSEIF (nu_minimum_wall.eq.2) THEN
					yplus = distance_to_bed*ust/nu_mol
					ekm(1,j,k) = MAX(ekm(1,j,k),nu_mol*rr(1,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2) 				
					distance_to_bed=Rp(2)-Ru(0) 
					yplus = distance_to_bed*ust/nu_mol
					ekm(2,j,k) = MAX(ekm(2,j,k),nu_mol*rr(2,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2) 	
				ELSEIF (nu_minimum_wall.eq.3) THEN 
					kp=MIN(kp+1,k1) 
					ww=0.5*(Wvel(2,j,k)+Wvel(2,j,k-1))
					vv=0.5*(Vvel(2,j,k)+Vvel(2,j-1,k))				
					absU2=sqrt(ww*ww+vv*vv)
					! ust*ust = nu_t*dUdz --> nu_t = ust*ust/dUdz 
					IF (absU2>1.1*absU) THEN
						nu_t = ust*ust/((absU2-absU)/(Rp(2)-Rp(1)))
						factor = nu_t/(0.5*(ekm(1,j,k)/rr(1,j,k)+ekm(2,j,k)/rr(2,j,k)))
						factor = MAX(factor,1.) 
						ekm(1,j,k)=factor*ekm(1,j,k)
						ekm(2,j,k)=factor*ekm(2,j,k)
					ELSE 
						yplus = distance_to_bed*ust/nu_mol
						ekm(1,j,k) = MAX(ekm(1,j,k),nu_mol*rr(1,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2) 				
						distance_to_bed=Rp(2)-Ru(0) 
				...
				ELSEIF (nu_minimum_wall.eq.4) THEN 
					! ust*ust = nu_t*dUdz --> nu_t = ust*ust/dUdz 
					nu_t = ust*ust/(MAX(absU,1.e-9)/distance_to_bed)
					factor = nu_t/(ekm(1,j,k)/rr(1,j,k))
					factor = MAX(factor,1.) 
					ekm(1,j,k)=factor*ekm(1,j,k)
				ELSEIF (nu_minimum_wall.eq.5) THEN 
					! OpenFOAM manner: tau = rho*ust*ust and ust=absU/Uplus and tau=nu_t*dUdz combined:
				...
				ELSEIF (nu_minimum_wall.eq.11) THEN 
					yplus = distance_to_bed*ust/nu_mol
					ekm(1,j,k) = nu_mol*rr(1,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2 
				!minimum eddy viscosity is Prandtl mixing length theory including Van Driest damping (factor A=19 and power 2 is found in Van Balen 2010 dissertation p15 and Catalano et al. 2010 circular cylinder flow)
				ELSEIF (nu_minimum_wall.eq.12) THEN
					yplus = distance_to_bed*ust/nu_mol
					ekm(1,j,k) = nu_mol*rr(1,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2 				
					distance_to_bed=Rp(2)-Ru(0) 
					yplus = distance_to_bed*ust/nu_mol
					ekm(2,j,k) = nu_mol*rr(2,j,k)*kappa*yplus*(1-exp(-yplus/19.))**2	
				ELSEIF (nu_minimum_wall.eq.13) THEN 
					kp=MIN(kp+1,k1) 
					ww=0.5*(Wvel(2,j,k)+Wvel(2,j,k-1))
					vv=0.5*(Vvel(2,j,k)+Vvel(2,j-1,k))				
			  ...
			  enddo
			 enddo 
		ELSEIF (monopile.eq.3.and.nu_minimum_wall.ge.1) then ! partial slip tau based on simplified TBL wall-law taking pressure gradient into account
			 do k=1,kmax 
			   do j=1,jmax
			    distance_to_bed = (Rp(1)-Ru(0))
				...
				ELSEIF (nu_minimum_wall.eq.4) THEN 
					...
					nu_t = ust*ust/(MAX(absU,1.e-9)/distance_to_bed)
					factor = nu_t/(ekm(1,j,k)/rr(1,j,k))
					IF (ust>0.5*absU) factor=1. !with monopile.eq.3 ust can be >0 with absU=0 because due to dPdx there can be tau without absU 
					factor = MAX(factor,1.) 
					ekm(1,j,k)=factor*ekm(1,j,k)
				ELSEIF (nu_minimum_wall.eq.5) THEN 
					! OpenFOAM manner: tau = rho*ust*ust and ust=absU/Uplus and tau=nu_t*dUdz combined:
...
!			ENDDO
	...
	real ctot,cref,pwr
	integer fracs_included(nfr_sand+nfr_silt)	
	real zb_C,distance_to_bed,nu_t,factor,absU2,tauu,tauv,tauw,uu,vv,ww,absU,ust,z0,yplus
...
!		u3 (i,j,k) = 0.5*(Wvel(i,j,k-1)+Wvel(i,j,k))
	  ...
	  enddo
	enddo
	u1 (1:imax,1:jmax,1:kmax) = 0.5*(Uvel(1:imax,1:jmax,1:kmax)+Uvel(0:imax-1,1:jmax,1:kmax)) !0.5*(Uvel(i-1,j,k)+Uvel(i,j,k))
	u2 (1:imax,1:jmax,1:kmax) = 0.5*(Vvel(1:imax,1:jmax,1:kmax)+Vvel(1:imax,0:jmax-1,1:kmax)) !0.5*(Vvel(i,j-1,k)+Vvel(i,j,k))
	u3 (1:imax,1:jmax,1:kmax) = 0.5*(Wvel(1:imax,1:jmax,1:kmax)+Wvel(1:imax,1:jmax,0:kmax-1)) !0.5*(Wvel(i,j,k-1)+Wvel(i,j,k))
...
!			 SabsSd33a=inv27*SUM(Sabs(iii,jjj,kkk)*Sd33(iii,jjj,kkk))		
			...
			L23 = u2u3a - u2a*u3a
			L33 = u3u3a - u3a*u3a
			M11 = Lmix2(i,j)*SabsSd11a - Lmix2hat(i,j)*Sabsa*Sd11a
			M12 = Lmix2(i,j)*SabsSd12a - Lmix2hat(i,j)*Sabsa*Sd12a
			M13 = Lmix2(i,j)*SabsSd13a - Lmix2hat(i,j)*Sabsa*Sd13a
			M22 = Lmix2(i,j)*SabsSd22a - Lmix2hat(i,j)*Sabsa*Sd22a
			M23 = Lmix2(i,j)*SabsSd23a - Lmix2hat(i,j)*Sabsa*Sd23a
			M33 = Lmix2(i,j)*SabsSd33a - Lmix2hat(i,j)*Sabsa*Sd33a

			Cd = 0.5*(L11*M11+L22*M22+L33*M33+2.*L12*M12+2.*L13*M13+2.*L23*M23) /
     &          (M11*M11+M22*M22+M33*M33+2.*M12*M12+2.*M13*M13+2.*M23*M23+1.e-12)
			Cd=MAX(0.,Cd)
			Cd = Cs_relax*sqrt(Cd)+(1.-Cs_relax)*Csgrid(i,j,k)
			Cd=MIN(0.23,Cd) !clip Cs between 0 and 0.23
			Csgrid(i,j,k)=Cd

		   ekm(i,j,k) = rr(i,j,k) * Lmix2(i,j) * Cd*Cd * Sabs(i,j,k)
...
!			ENDDO		
     ...
     &					,status,ierr)	 
				...
				IF (periodicy.eq.1) THEN !periodic lateral boundaries 
				  do k=1,kmax/px 
				   do i=1,imax
     ...
     &			rhssy(1:px*jmax),px*jmax,pery)
				   enddo
				  enddo
				ELSE !free slip lateral boundaries d.dn=0 at both ends 
				  do k=1,kmax/px 
				   do i=1,imax
					 do j=1,px*jmax !0,px*jmax+1 
					   jm=j-1 !MAX(0,j-1)
     ...
     &			cccy(1:px*jmax),rhssy(1:px*jmax),px*jmax,pery)	
				...
				IF (periodicx.eq.1) THEN !periodic bc
				 do k=1,kmax 
				   do j=1,jmax 
					 do i=1,imax !0,i1
					   im=i-1 !MAX(0,i-1)
					   ip=i+1 !MIN(i1,i+1)
					   ekm_min=0.5*(Diffcof3(im,j,k)+Diffcof3(i,j,k)) !*MIN(fc_global(i,j+rank*jmax,k),fc_global(im,j+rank*jmax,k))
					   ekm_plus=0.5*(Diffcof3(ip,j,k)+Diffcof3(i,j,k)) !*MIN(fc_global(ip,j+rank*jmax,k),fc_global(i,j+rank*jmax,k))
             ...
             do j=1,jmax
               do i=1,imax
                 do k=1,kmax !0,k1
				   km=k-1 !MAX(0,k-1)
				   kp=k+1 !MIN(k1,k+1)
				   ekm_min=0.5*(Diffcof3(i,j,km)+Diffcof3(i,j,k)) !*MIN(fc_global(i,j+rank*jmax,k),fc_global(i,j+rank*jmax,km))
				   ekm_plus=0.5*(Diffcof3(i,j,kp)+Diffcof3(i,j,k)) !*MIN(fc_global(i,j+rank*jmax,kp),fc_global(i,j+rank*jmax,k))
				 ...
				 ccc(kmax)=0.
				 rhss(1:kmax)=TKEnew(i,j,1:kmax) 
				 CALL solve_tridiag(TKEnew(i,j,1:kmax),aaa(1:kmax),bbb(1:kmax),ccc(1:kmax),rhss(1:kmax),kmax) 
               enddo
             enddo			 
		ENDIF	 
     ...
     &			cccy(1:px*jmax),rhssy(1:px*jmax),px*jmax,pery)	
             ...
             do j=1,jmax
               do i=1,imax
				 ...
				 bbb(kmax)=bbb(kmax)+ccc(kmax)
				 ccc(kmax)=0.
				 rhss(1:kmax)=EEEnew(i,j,1:kmax) 
				 CALL solve_tridiag(EEEnew(i,j,1:kmax),aaa(1:kmax),bbb(1:kmax),ccc(1:kmax),rhss(1:kmax),kmax) 
               enddo
             enddo			 
		ENDIF
...
!		ENDIF
!		ENDDO ! bedplume loop		
		if (slip_bot.ge.1.and.wallup.ne.1) then !partial slip wall below and free surface bc up 
				IF (IBMorder.eq.2) THEN
					DO i=0,i1
					  DO j=0,j1
						zb_W=zbed(i,j)
						...
						distance_to_bed=(REAL(kpp)-0.5)*dz-zb_W !as start use first cell (0.5-1)*dz distance from bed 
...
!				ENDIF 		
			  do i=1,imax
				...
				ELSE 
					...
					vv=0.5*(Vvel(i,j,kppN)+Vvel(i,j-1,kppS))-Vbot_TSHD(j)
...
!	  IF (monopile>0)  ! dy,dx is too coarse at inflow with monopile>0 for SEM to be effective + too much to be changed in rest of code 
!		  yymin=pi/2.
!		  yymax=1.5*pi 
!		  ySEM2=(yymax-yymin)*ySEM2+yymin !angle describing left half circle at inflow
!		  xxmin=-0.6*(depth-bc_obst_h) !boxsize
!		  xxmax=0.6*(depth-bc_obst_h) !boxsize
!		  xSEM2=(xxmax-xxmin)*xSEM2+xxmin	
!		  !make xSEM2,ySEM2 as x,y coordinates in [m]:
!		  xSEM2 = Rp(i1)*cos(ySEM2) + schuif_x + xSEM2  ! follow half circle of inflow boundary with random +/- boxsize_x
!		  ySEM2 = Rp(i1)*sin(ySEM2) 
!	  ELSE 
		  yymin=Rp(0)*sin_vt(0)-0.22*(depth-bc_obst_h) !boxsize
		  yymax=Rp(0)*sin_vt(jmax*px)+0.22*(depth-bc_obst_h) !boxsize
...
!           endif
	...
	do iii=1,iimax
	  ii=ind(iii)
	  kminSEM=INT(FLOOR((zSEM2(ii)-lmzSEM2(ii))/dz))
	  kmaxSEM=INT(CEILING((zSEM2(ii)+lmzSEM2(ii))/dz))+MAXVAL(kbed0) !add maximum kbed of this processor is sufficient; only interactions for grid-cells on this specific processor are needed
	  kminSEM=MAX(kminSEM,1)
	  kminSEM=MIN(kminSEM,kmax)
	  kmaxSEM=MAX(kmaxSEM,1)
	  kmaxSEM=MIN(kmaxSEM,kmax)
